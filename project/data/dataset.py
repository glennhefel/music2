...
class PreprocessedAudioDataset(Dataset):
    def __init__(self, manifest_path: str | Path, handcrafted_csv: str | Path | None = None) -> None:
        ...
        if handcrafted_csv is not None:
            self.handcrafted_features = self._load_handcrafted_features(handcrafted_csv)

    @staticmethod
    def _load_handcrafted_features(path: str | Path) -> dict[tuple[str, int], np.ndarray]:
        features = {}
        with Path(path).open(newline="", encoding="utf-8") as file:
            for row in csv.DictReader(file):
                ...
                features[(recording_stem, int(chunk_text))] = np.asarray([float(row[name]) for name in names], dtype=np.float32)
        if not features:
    ...
    def _handcrafted_for_record(self, record: dict) -> np.ndarray:
        ...
        chunks = [self.handcrafted_features.get((recording_stem, segment_index * 4 + offset)) for offset in range(4)]
